...
<body>
	...
	<%@ include file="../../include/mypage_left.jsp" %>
		<div class="content">
			...
			구독한 상품이 없습니다.
			<div class="jb-division-line4"></div><br><br>
			<h1>주문조회</h1>
			<div class="jb-division-line4"></div><br>
			
			<!-- 날짜 조회 -->
			<div class="orderhistory">
				<form id="dateForm" name="dateForm" action="/coffeedao/orderhistory.da" method="POST">
					<label for="DATE_START" style="font-size: 16px">시작 날짜</label>&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="date" id="DATE_START" name="DATE_START" value="${start}">&nbsp;&nbsp;&nbsp;&nbsp;
					<label for="DATE_END" style="font-size: 16px">종료 날짜</label>&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="date" id="DATE_END" name="DATE_END" value="${end}">
					<button type="submit" class="send_bt orderhistory-right"><a>조회</a></button><br>
				</form>
			</div>
			<div class="jb-division-line4"></div><br>
		<c:forEach items="${orderList}" var="list">
			...
			<div class="orderhistory">
				<div class="orderhistory-left">
					<div class="orderhistory-img"><img class="img_pro"src="${pageContext.request.contextPath}/resources/img/${list.FILE_STD}"></div>
				</div>
				<div class="orderhistory-left orderhistory-text">
					${list.ORDER_ITEM}<br>
					${list.ORDER_PRICE}원<br>
					${list.ORDER_COUNT}개<br>
					${list.ORDER_STATE}<br>
				...
				<div class="orderhistory-left orderhistory-text2">
					${list.SELL_NAME}<br>
					${list.SELL_PHONE}<br>
					<div class="orderhistory_bt"><a href="inquiryWriteForm.da">문의하기</a></div>
				</div>
				<!-- 주문완료, 배송중, 배송완료 -->
				...
				<c:choose>
					<c:when test="${list.ORDER_STATE == '주문완료'}">
						...
						<form action="/coffeedao/orderState.da" method="post">
							...
							<button type="submit" class="orderhistory_bt" onclick="alert('판매자와 협의 없이 신청 시에는 주문취소 처리가 되지 않을 수 있습니다.')"><a>주문취소</a></button><br>
						</form>
					</c:when>
					<c:when test="${list.ORDER_STATE == '배송중'}">
						<div class="orderhistory_bt"><a href="#">배송조회</a></div><br>
						<%-- <div class="orderhistory_bt"><a target="_blank" onclick="openPop(${list.ORDER_ID})">주문취소</a></div><br> --%>
						<form action="/coffeedao/orderState.da" method="post">
							<input type="hidden" name="ORDER_ID" value="${list.ORDER_ID}">
							<input type="hidden" name="ORDER_STATE" value="주문취소">
						...
						<form action="/coffeedao/orderState.da" method="post">
							...
							<button type="submit" class="orderhistory_bt" onclick="alert('구매확정 되었습니다.')"><a>구매확정</a></button><br>
						</form>
					</c:when>
					...
					<c:when test="${list.ORDER_STATE == '구매확정'}">
						<br><br>
						<div class="orderhistory_bt"><p style="color: #666666; font-size: 14px; font-family: 'Poppins', sans-serif; line-height: 1.80857; font-weight: normal;">구매확정</p></div><br>
					</c:when>
					<c:when test="${list.ORDER_STATE == '주문취소'}">
						<br><br>
						<div class="orderhistory_bt"><p style="color: #666666; font-size: 14px; font-family: 'Poppins', sans-serif; line-height: 1.80857; font-weight: normal;">취소처리</p></div><br>
					</c:when>
					<c:when test="${list.ORDER_STATE == '취소완료'}">
						<br><br>
						<div class="orderhistory_bt"><p style="color: #666666; font-size: 14px; font-family: 'Poppins', sans-serif; line-height: 1.80857; font-weight: normal;">취소완료</p></div><br>
					</c:when>
				</c:choose>
				</div>
			</div>
